...
import {FewETHHook} from "../../src/hooks/FewETHHook.sol";
import {IFewWrappedToken} from "../../src/interfaces/external/IFewWrappedToken.sol";
import "../mocks/MockFwWETH.sol";

contract FewWETHHookTest is Test, Deployers {
    using PoolIdLibrary for PoolKey;
    using CurrencyLibrary for Currency;

    FewETHHook public hook;
    WETH public weth;
    MockFwWETH public fwWETH;
    PoolKey poolKey;
    ...
    function setUp() public {
        ...
        deployCodeTo("FewETHHook", abi.encode(manager, address(weth), fwWETH), address(hook));

        // Create pool key for ETH/fwWETH
        poolKey = PoolKey({
            currency0: CurrencyLibrary.ADDRESS_ZERO,
            currency1: Currency.wrap(address(fwWETH)),
        ...
        // Provide fwWETH to alice and WETH to the hook and fwWETH contract for testing
        deal(address(fwWETH), alice, 100 ether);
        deal(address(weth), address(hook), 100 ether);
        deal(address(weth), address(fwWETH), 100 ether);  // Provide WETH to the MockFwWETH contract to support unwrapping
    ...
    function test_initialization() public view {
        ...
        assertEq(address(hook.fwWETH()), address(fwWETH));
        assertEq(Currency.unwrap(hook.wrapperCurrency()), address(fwWETH));
    ...
    function test_wrapETH() public {
        ...
        uint256 aliceFwWethBalanceBefore = fwWETH.balanceOf(address(alice));
        uint256 managerEthBalanceBefore = address(manager).balance;
        uint256 managerFwWethBalanceBefore = fwWETH.balanceOf(address(manager));
        ...
        swapRouter.swap{value: wrapAmount}(
            ...
            IPoolManager.SwapParams({
                zeroForOne: true, // ETH (0) to fwWETH (1)
        ...
        uint256 aliceFwWethBalanceAfter = fwWETH.balanceOf(address(alice));
        uint256 managerEthBalanceAfter = address(manager).balance;
        uint256 managerFwWethBalanceAfter = fwWETH.balanceOf(address(manager));
        assertEq(aliceEthBalanceBefore - aliceEthBalanceAfter, wrapAmount);
        assertEq(aliceFwWethBalanceAfter - aliceFwWethBalanceBefore, wrapAmount);
        assertEq(managerEthBalanceBefore, managerEthBalanceAfter);
        assertEq(managerFwWethBalanceBefore, managerFwWethBalanceAfter);
    }

    function test_unwrapFwWETH() public {
        uint256 unwrapAmount = 1 ether;

        // Unwrap fwWETH back to ETH
        uint256 aliceEthBalanceBefore = alice.balance;
        uint256 aliceFwWethBalanceBefore = fwWETH.balanceOf(address(alice));
        uint256 managerEthBalanceBefore = address(manager).balance;
        uint256 managerFwWethBalanceBefore = fwWETH.balanceOf(address(manager));
        ...
        vm.startPrank(alice);
        fwWETH.approve(address(swapRouter), type(uint256).max);
        PoolSwapTest.TestSettings memory testSettings =
            PoolSwapTest.TestSettings({takeClaims: false, settleUsingBurn: false});
        ...
        swapRouter.swap(
            poolKey,
            IPoolManager.SwapParams({
                zeroForOne: false, // fwWETH (1) to ETH (0)
        ...
        assertEq(aliceFwWethBalanceBefore - aliceFwWethBalanceAfter, unwrapAmount);
        assertEq(managerEthBalanceBefore, managerEthBalanceAfter);
        assertEq(managerFwWethBalanceBefore, managerFwWethBalanceAfter);
    }
    ...
    function test_unwrapFwWETH_exactOut() public {
        uint256 unwrapAmount = 1 ether;

        // Unwrap fwWETH back to ETH with exact output
        uint256 aliceEthBalanceBefore = alice.balance;
        uint256 aliceFwWethBalanceBefore = fwWETH.balanceOf(address(alice));
        uint256 managerEthBalanceBefore = address(manager).balance;
        uint256 managerFwWethBalanceBefore = fwWETH.balanceOf(address(manager));
        ...
        swapRouter.swap(
            ...
            IPoolManager.SwapParams({
                zeroForOne: false, // fwWETH (1) to ETH (0)
                amountSpecified: int256(unwrapAmount), // Positive for exact output
                sqrtPriceLimitX96: TickMath.MAX_SQRT_PRICE - 1
            }),
            testSettings,
            ""
    ...
    function test_revertInvalidPoolInitialization() public {
        ...
        // sort tokens
        (Currency currency0, Currency currency1) = address(randomToken) < address(fwWETH)
            ? (Currency.wrap(address(randomToken)), Currency.wrap(address(fwWETH)))
            : (Currency.wrap(address(fwWETH)), Currency.wrap(address(randomToken)));
        invalidKey =
    ...
    // add some unrelated ETH and fwWETH liquidity that the hook can use
    function _addUnrelatedLiquidity() internal {
        // Create a hookless pool key for ETH/fwWETH
        ...
        // Use deal directly for fwWETH tokens instead of wrapping
        deal(address(fwWETH), address(this), 100 ether);
        fwWETH.approve(address(modifyLiquidityRouter), type(uint256).max);
